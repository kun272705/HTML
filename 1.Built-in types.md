...
| Element | Description |
| --- | --- |
| \<embed\> | Embeds external content at the specified point in the document. This content is handled by the browser or an external application. |
| \<iframe\> | Represents a nested browsing context, embedding another document into the current one. |
...
| \<geolocation\>  | Creates an interactive control for the user to share their geolocation data with the page. |
